...
test("ships the curated, complete wiki and sourced build catalogues with the full remembrance route", async () => {
  ...
  assert.match(data, /Verified data baseline: patch 1\.17/);
  assert.match(page, /The Tarnished Pack origins, eight armaments, four armour sets, and two altered armour variants are included/);
...
test("includes a read-only LAN follower and Elden Ring build filters", async () => {
  ...
  assert.match(server, /\/api\/join/);
  assert.match(server, /tarnished-together-join/);
  assert.match(server, /tarnished-together-session/);
  assert.match(server, /\/api\/claim-player/);
  assert.match(server, /claimCodeFor\(playerId\)/);
  assert.match(server, /const playerId = session\.playerId/);
  assert.match(server, /mutateState/);
  ...
  assert.match(server, /commitExpedition\(next, "player"\)/);
  ...
  assert.match(page, /Buff routine/);
  assert.match(page, /buffSupportItems/);
  assert.match(page, /Requires \$\{buff\.requirements/);
  ...
  assert.match(page, /Mutually exclusive rewards are called out/);
