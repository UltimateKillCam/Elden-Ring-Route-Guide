...
const curatedBuilds: Build[] = [
  build("quality-knight", "Quality Knight", "STR / DEX", "Frontline", "Keep one Quality-infused weapon equipped at a time so the same Strength and Dexterity plan carries every stage. Great Épée heavy attacks add broad sweeps to the thrusting moveset, while Milady's Wing Stance offers either a three-slash pressure string or a leaping thrust for a larger punish. Splitting levels across two damage stats ramps more slowly than a focused infusion, and the kit has no native distance tool.", ["Longsword", "Claymore", "Quality Great Épée", "Milady + Wing Stance"], ["quality", "sword", "guard"], "Easy", undefined, undefined, "Stance damage"),
  ...
  build("colossal-hammer", "Colossal Hammer", "STR", "Breaker", "Charged hammer blows accumulate stance damage until a boss collapses for a critical. Barbaric Roar turns the early club's heavy attacks into forceful combos; Golden Land, Endure and Smithing Art Spears give the later weapons distinct tools instead of repeating that roar. Every stage demands discipline because the long windups are safe only during real openings.", ["Large Club", "Great Club", "Giant-Crusher", "Anvil Hammer"], ["strength", "hammer", "stance"], "Easy", undefined, undefined, "Stance damage"),
  ...
  build("powerstance-spears", "Twin-Spear Jump Attacker", "DEX / STR", "Melee", "Stay mobile with two spears powerstanced, using measured pokes until an opening justifies a simultaneous dual-weapon strike. Ancient Lightning Spear punishes distant phases, while Spear Talisman counter-hits and consecutive-attack bonuses reward disciplined pressure. The narrow moveset struggles when a fast enemy denies the spacing needed to reset.", ["Short Spear + Partisan", "Cross-Naginata + Spiked Spear", "Keen Cross-Naginata pair", "Messmer Soldier's Spear pair"], ["quality", "spear", "jump"], "Moderate", undefined, { label: "EIP PvE: Dual Wielding Spear Build", url: BUILD_SOURCES.eip }, "Jump attacks", "Vagabond"),
  build("guardian-swordspear", "Dual Guardian Swordspear", "DEX", "Melee", "Powerstance two Keen Guardian's Swordspears and use their unusually fast halberd strings to build consecutive-hit bonuses. Jumping attacks land both weapons together, while Repeating Thrust reaches the hit-count bonuses quickly during longer openings. The engine loses much of its payoff against evasive bosses that will not allow sustained contact.", ["Halberd", "Guardian's Swordspear", "Keen Guardian's Swordspear pair", "Keen Guardian's Swordspear pair"], ["dexterity", "halberd", "multihit"], "Moderate", undefined, { label: "EIP PvE: Dual Guardian Swordspear", url: BUILD_SOURCES.eip }, "Light attacks", "Samurai"),
  build("blasphemous-vicar", "Blasphemous Blade Vicar", "STR / FAI", "Melee", "Taker's Flames is the main punish: its slow, armoured fire wave deals heavy damage and restores health when it connects, while the blade also heals when nearby enemies die. Shard of Alexander, fire bonuses and FP-cost reduction reinforce that loop. The long startup makes careless casts easy to punish, so the healing should sustain good trades rather than excuse them.", ["Lordsworn's Greatsword + Sacred Blade", "Magma Wyrm's Scalesword", "Blasphemous Blade", "Blasphemous Blade + Talisman of the Dread"], ["strength", "faith", "fire"], "Easy", "Rya and Rykard progression", { label: "PCGamesN: Blasphemous Blade build", url: BUILD_SOURCES.pcGamesN }, "Skill damage", "Hero"),
  build("halo-reaper", "Halo Scythe Disciple", "DEX / FAI", "Ranged", "Use the reaper's long, blood-loss sweeps in melee and cast Miquella's Ring of Light whenever spacing or co-op aggro makes range safer. The inexpensive ring can be chained, but both scythe recoveries and projectile casts require deliberate timing in cramped fights. Holy-resistant bosses reduce the ranged payoff without removing the weapon's physical and bleed pressure.", ["Winged Scythe", "Halo Scythe", "Halo Scythe + Grave Scythe", "Halo Scythe + Barbed Staff-Spear"], ["dexterity", "faith", "reaper", "ranged"], "Moderate", undefined, { label: "wiki.gg: Halo Scythe", url: "https://eldenring.wiki.gg/wiki/Halo_Scythe" }, "Ranged attacks", "Confessor"),
  build("nebula-astel", "Wing of Astel Nebula", "DEX / INT", "Melee", "Bait a large target into committing, then place Nebula close enough for several delayed starbursts to overlap its hitbox. The sword's uncharged and charged heavy attacks throw free magic beams, adding ranged pressure and stance damage without spending FP. Golden Vow, Howl of Shabriri and Terra Magica create a potent but lengthy boss setup, and small mobile enemies can escape both the field and the buff window.", ["Magic Shamshir", "Magic Shamshir", "Wing of Astel + Finger Seal", "Wing of Astel + Finger Seal"], ["dexterity", "intelligence", "curved", "stance"], "Moderate", "Ranni progression", { label: "PCGamesN: Wing of Astel build", url: BUILD_SOURCES.pcGamesN }, "Stance damage", "Prisoner"),
  build("poison-lizard", "Poison Lizard Greatsword", "DEX / ARC", "Melee", "Apply poison with normal swings and the Lizard Greatsword's forked-tongue heavy projectile, whose weapon and projectile hits both carry status buildup. Once poison is active, The Poison Flower Blooms Twice turns that setup into a burst punish while poison-triggered gear raises attack power. Immune enemies reduce the plan to its physical greatsword fundamentals.", ["Claymore + Poisonous Mist", "Iron Greatsword + Poisonous Mist", "Poison Flamberge", "Poison Lizard Greatsword"], ["dexterity", "arcane", "poison", "charged"], "Moderate", undefined, { label: "PCGamesN: Poison Lizard build", url: BUILD_SOURCES.pcGamesN }, "Charged attacks", "Bandit"),
  build("thorn-sorcerer", "Briar and Thorn Sorcerer", "INT / FAI / ARC", "Ranged", "Thorn sorceries spend health as well as FP, sending blood-loss briars through enemies or forming an immediate point-blank wall. Faith meets the spell requirements, the chosen staff supplies sorcery scaling, and Arcane can raise hemorrhage buildup when the catalyst scales with it. Long casts and self-damage make every trade costly, especially before Impenetrable Thorns becomes available in Shadow Keep.", ["Meteorite Staff", "Academy Glintstone Staff", "Staff of the Guilty + Briars of Punishment", "Maternal Staff + Impenetrable Thorns"], ["intelligence", "faith", "arcane", "sorcery", "bleed"], "Advanced", "Ymir quest", { label: "PCGamesN: Bleeding Thorns Mage", url: BUILD_SOURCES.pcGamesN }, "Spell damage", "Astrologer"),
  build("lightning-perfumer", "Lightning Perfume Prophet", "DEX / FAI", "Ranged", "Lightning incantations carry the base game; in the DLC, aim Rolling Sparks at a target's feet so its successive perfume explosions concentrate in one area. Knight's Lightning Spear handles longer range and Blasphemous Blade can provide a healing melee fallback, while Perfumer's Talisman and Shard of Alexander reinforce the bottle skill. The close-range casting animation remains punishable, and the old launch-era perfume burst has been nerfed.", ["Finger Seal + Lightning Spear", "Finger Seal + Lightning Spear", "Blasphemous Blade", "Lightning Perfume Bottle + Blasphemous Blade"], ["dexterity", "faith", "lightning", "perfume", "ranged"], "Moderate", undefined, { label: "PCGamesN: Lightning Perfume Bottle Prophet", url: BUILD_SOURCES.pcGamesN }, "Ranged attacks", "Prophet"),
  ...
  build("sword-of-darkness", "Sword of Darkness Knight", "STR / FAI", "Melee", "Blanket nearby enemies with the Sword of Darkness's dark fog, dealing holy damage and lowering their holy negation before following with sword strikes or incantations. Golden Vow and lightning incantations broaden the Faith side while Blessed Blue Dew supports the skill's FP demand. The central weapon requires the Stone-Sheathed Sword altar detour, so the base game uses an ordinary sacred sword-and-seal route.", ["Sacred Lordsworn's Straight Sword", "Sacred Lordsworn's Straight Sword", "Coded Sword", "Sword of Darkness + Dryleaf Seal"], ["strength", "faith", "holy", "skill"], "Moderate", "Stone-Sheathed Sword altar route", { label: "PC Gamer: Unholy Knight", url: BUILD_SOURCES.pcGamerDlc }, "Skill damage", "Hero"),
  build("carian-shield-sorcerer", "Carian Shield Sorcerer", "STR / DEX / INT", "Hybrid", "Alternate thrusts with sorceries cast from the Carian Sorcery Sword's heavy input, keeping a melee answer ready without swapping weapons. The Carian Thrusting Shield handles guarded pressure and counter-pokes, while Ymir's sorceries cover range; Beloved Stardust speeds casting at the cost of damage absorption. Both signature weapons arrive in the DLC, so the base game develops the same spellblade and shield fundamentals with conventional gear.", ["Estoc + Demi-Human Queen's Staff", "Carian Knight's Sword + staff", "Loretta's War Sickle + Carian Regal Scepter", "Carian Sorcery Sword + Carian Thrusting Shield"], ["strength", "dexterity", "intelligence", "sorcery", "guard"], "Advanced", undefined, { label: "PC Gamer: Carian Spellblade", url: BUILD_SOURCES.pcGamerDlc }, "Guard counters", "Prisoner"),
  build("wing-stance-duelist", "Wing Stance Court Duelist", "STR / DEX", "Melee", "Hold Wing Stance and choose between its rapid slash string for sustained pressure or the leaping thrust for reach, poise damage and larger punish windows. Rellana's Cameo rewards a deliberate stance hold, while the Main-gauche turns successful parries into dagger criticals. With no shield or status engine, success rests on stance timing, spacing and reliable parries.", ["Rapier", "Rogier's Rapier", "Cleanrot Knight's Sword", "Milady + Wing Stance / Main-gauche"], ["quality", "light greatsword", "critical", "stance"], "Advanced", undefined, { label: "PC Gamer: Courtly Duelist", url: BUILD_SOURCES.pcGamerDlc }, "Critical attacks", "Vagabond"),
  build("red-bear", "Red Bear Berserker", "STR / DEX", "Melee", "Stay close with Red Bear's Claw flurries so consecutive hits, bleed triggers and recent kills keep separate attack bonuses active. Red Bear Hunt is the committed burst option, with FP reduction or skill damage support chosen according to how often it can land. Light armour and bleed-immune enemies expose the build's main weakness: much of its momentum disappears after one forced disengagement.", ["Hookclaws", "Hookclaws", "Star Fist", "Red Bear's Claw"], ["quality", "claw", "multihit", "bleed"], "Advanced", undefined, { label: "PC Gamer: Bearserker", url: BUILD_SOURCES.pcGamerDlc }, "Light attacks", "Vagabond"),
  build("pure-priestess", "Co-op Erdtree Priestess", "FAI", "Ranged", "Adapt the incantation bar to each encounter instead of forcing one damage type: offensive elemental spells handle range, while physical, fire, magic, lightning and holy fortifications answer specific bosses. Healing supports allies and spirit ashes without creating an MMO-style tank role, and a Dragon Communion Seal can lend its passive while a pure-Faith seal performs the cast. The broad toolkit trades peak damage for flexibility and careful memory-slot planning.", ["Finger Seal + Catch Flame", "Godslayer's Seal", "Erdtree Seal", "Dryleaf Seal + Knight's Lightning Spear"], ["faith", "caster", "coop", "ranged"], "Moderate", undefined, { label: "EIP PvE: Pure Faith Priestess", url: BUILD_SOURCES.eip }, "Spell damage", "Prophet"),
  build("magma-sorcerer", "Gelmir Magma Sorcerer", "INT / FAI", "Ranged", "Pool magma under large targets with Magma Shot and Roiling Magma, then use ordinary glintstone projectiles when their short reach is unsafe. Chargeable spells benefit from Godfrey Icon, twin Gelmir Glintstone Staves amplify the magma school, and a melee sidearm covers enemies that outrun the pools. Fire immunity and highly aggressive bosses are hard counters because they remove either the damage type or the casting time the loop needs.", ["Meteorite Staff + Magic Longsword", "Gelmir Glintstone Staff", "Dual Gelmir Glintstone Staff", "Dual Gelmir Glintstone Staff + Rellana's Twin Blades"], ["intelligence", "faith", "magma", "caster"], "Advanced", "Volcano Manor progression", { label: "EIP PvE: Magmage", url: BUILD_SOURCES.eip }, "Spell damage", "Astrologer"),
  build("black-blade-colossus", "Maliketh Black Blade", "STR / FAI", "Melee", "Use Maliketh's Black Blade as a durable Strength/Faith colossal sword, reserving Destined Death for openings that can absorb its committed animation. Siluria's Tree, Ordovis's Greatsword and Death Knight's Longhaft Axe can rotate through the same stat plan when a different moveset is preferable. The published setup values variety and fashion over a fully optimised damage stack, and its signature weapon cannot exist until Maliketh falls.", ["Sacred Claymore", "Golden Halberd", "Maliketh's Black Blade", "Maliketh's Black Blade + Black Blade incantation"], ["strength", "faith", "colossal", "destined death"], "Advanced", undefined, { label: "EIP PvE: Black Blade STR/Faith", url: BUILD_SOURCES.eip }, "Skill damage", "Hero"),
  build("messmer-impaler", "Messmer Impaler", "DEX / FAI", "Ranged", "Control spacing with great-spear thrusts, then commit to Messmer's Assault only when its full chain and final slam can land. Charged heavy attacks throw the spear and ignite on impact, while Fire Serpent tracks targets during gaps the melee moveset cannot cover. The weapon's high Dexterity requirement and late remembrance gate delay the complete setup, and fire-resistant enemies reduce half of its pressure.", ["Short Spear + Flame Sling", "Cross-Naginata + Black Flame", "Bolt of Gransax", "Spear of the Impaler + Fire Serpent"], ["dexterity", "faith", "spear", "fire", "ranged"], "Advanced", undefined, { label: "wiki.gg: Spear of the Impaler", url: "https://eldenring.wiki.gg/wiki/Spear_of_the_Impaler" }, "Skill damage", "Confessor"),
];
